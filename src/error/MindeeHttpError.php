...
class MindeeHttpException extends MindeeException
{
    ...
    public function __construct(array $http_error, string $url, int $code)
    {
        ...
        parent::__construct("$url $this->status_code HTTP error: $this->api_details - $this->api_message");
    }
}

function create_error_obj($response): array
{
    ...
    if (array_key_exists('api_request', $response) && array_key_exists('api_request', $response['api_request'])) {
...
function handle_error(string $url, array $response, int $code): MindeeHttpException
{
