...
class CustomLineV1
{
    ...
    public function updateFields(string $field_name, ListFieldValueV1 $field_value)
    {
        ...
        $this->fields[$field_name] = new ListFieldValueV1([
    ...
    public static function prepare(
        ...
        $current_line = new CustomLineV1($current_line_number);
        ...
        for ($i = 1; $i < count($anchor_field->values); ++$i) {
            ...
            if (!CustomLineV1::isBBoxInLine(
                $current_line,
                $current_field_box,
                $height_line_tolerance
            )) {
                array_push($lines_prepared, $current_line);
                ++$current_line_number;
                $current_line = new CustomLineV1($current_line_number);
                $current_line->bbox->extendWith($current_value->polygon);
            }
    ...
    public static function getLineItems(
        ...
        $anchor = CustomLineV1::findBestAnchor($anchors, $fields_to_transform);
        if (!$anchor) {
        ...
        $lines_prepared = CustomLineV1::prepare(
            $anchor,
            $fields_to_transform,
            $height_line_tolerance
        );
            ...
            foreach ($fields_to_transform as $field_name => $field) {
                foreach ($field->values as $list_field_value) {
                    ...
                    if (
                        ...
                        $current_line->updateFields($field_name, $list_field_value);
                    }
                }
            }
        }
