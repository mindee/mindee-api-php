...
class UsMailV2Test extends TestCase
{
    ...
    {
        $productDir = (getenv('GITHUB_WORKSPACE') ?: ".") . "/tests/resources/products/us_mail/response_v2/";
        $completeDocFile = file_get_contents($productDir . "complete.json");
        $emptyDocFile = file_get_contents($productDir . "empty.json");
        $completeDocJSON = json_decode($completeDocFile, true);
        $emptyDocJSON = json_decode($emptyDocFile, true);
        $this->completeDoc = new Document(UsMail\UsMailV2::class, $completeDocJSON["document"]);
        $this->emptyDoc = new Document(UsMail\UsMailV2::class, $emptyDocJSON["document"]);
